...
### Added
- `scripts/indexability_audit.py` → `docs/INDEXABILITY.md` — single hard CI gate that asserts every indexable HTML page has:
  - explicit `<meta name=robots>` with `max-image-preview:large`
  - `<link rel=canonical>`
  - JSON-LD block
  - non-empty `<title>`
  - meta description ≥ 30 chars
  - presence in `sitemap.xml`
  - and that `robots.txt` lists the sitemap

### Changed
- `site-map/index.html` robots meta upgraded to `index,follow,max-image-preview:large` (was `index,follow`); fixed in `seo_optimize.py` template too
- `site_maintenance.py` now runs `indexability_audit.py` as a hard CI step

### Result
- **73 / 73 indexable HTML pages pass every criterion** — 0 failures
- Sitemap coverage: 73 HTML + 11 supporting (data/feeds/llms/ai) = 84 total
- Homepage fingerprint guard still green

## [2026.05.28a] — Professional repo posture

### Added
- `.editorconfig` — cross-IDE indentation + EOL consistency
- `.github/dependabot.yml` — weekly GitHub Actions security updates (capped at 5 PRs)
- `.github/CODEOWNERS` — homepage + scripts paths route to @multilogin-labs
- `.github/FUNDING.yml` — Sponsor button → SAAS50 verifier
- `.github/ISSUE_TEMPLATE/config.yml` — disable blank issues + route to Discussions / SAAS50 verifier
- `.github/ISSUE_TEMPLATE/feature_request.yml` — structured feature proposal form
- `pyproject.toml` — ruff 0.5.7 config (line-length 110, py311 target, project metadata)
- `MAINTAINERS.md` — merge-rights guide, release cadence, homepage policy
- `ROADMAP.md` — quarterly outlook + explicit non-goals
- `AUTHORS.md` — contributor + acknowledgments policy

### Changed
- `.github/workflows/ci.yml` — concurrency cancel-in-progress, pip cache, dedicated `lint` job (ruff) + `validate` + `indexnow`, artifacts upload of all `docs/*-REPORT.md` for transparency
- Sitemap-drift check is now a **hard error** (not just warning)
- All `scripts/*.py` pass `ruff check` (13 issues fixed sitewide — unused vars, ambiguous names, f-string noise, nested ifs, ternary opportunities)
- Moved stray `9004568_Multilogin-promo-coupon.png` from repo root → `assets/img/legacy/` (root is now docs + automation only)
- README: added Ruff / Schema / A11y badges, mermaid architecture diagram, links to roadmap/maintainers/reports

### Result
- Repo passes GitHub community standards checklist
- CI: 3 jobs (lint → validate → indexnow), cancellable, cached
- Lint: 0 errors across all 30+ scripts
- Homepage fingerprint guard still green

## [2026.05.27f] — Sitewide schema + a11y guardrails

### Added
- `scripts/normalize_jsonld.py` — parses every JSON-LD block, validates JSON, re-emits with 2-space indent; homepage excluded from rewrites
- `scripts/schema_coverage.py` → `docs/SCHEMA-COVERAGE.md` (indexable count, type distribution, missing schema / breadcrumb lists)
- `scripts/a11y_check.py` — sitewide guard: `<html lang>`, single `<h1>`, `.skip-link`, `<img alt>` (CI-grade fail)
- JSON-LD on `/site-map/` (CollectionPage + BreadcrumbList), regenerated by `seo_optimize.py`
- Catalog hub schema upgraded to CollectionPage + BreadcrumbList + ItemList (top 7 catalog entries)

### Result
- 70+ files normalized — fixed runaway-indent JSON-LD corruption (e.g. `guides/index.html` reduced ~7 KB)
- 73 indexable pages, **100% JSON-LD coverage**, **100% BreadcrumbList coverage**
- Type mix: 73 BreadcrumbList · 53 FAQPage · 46 Article · 21 TechArticle · 9 CollectionPage · 4 WebApplication · 4 ItemList · 2 Offer · 2 HowTo · 1 VideoObject · 1 ImageObject · …
- A11y: 0 lang/h1/skip-link/alt violations
- Homepage fingerprint guard still green

## [2026.05.27e] — Promo pages: unique research content

### Changed
- All 22 `promo/*` pages rewritten from generic template to vendor-specific research:
  - Unique title/description/lead per vendor (no near-duplicate content)
  - Per-vendor `risk_short`, `category`, `validation_question`, `multilogin_edge`
  - New sections: Verdict · Vendor profile · Validation question · Comparison table vs Multilogin · Activation checklist · FAQ (with FAQPage JSON-LD) · Related vendor reviews
  - Affiliate disclosure paragraph in hero
  - Cross-links to 4 sibling promo pages (raises inlinks from 1 → 6+ each)
- `promo/index.html` cards now include "Read full {vendor} review" link (extra inlink path)

### Result
- Index health: low-inlink URLs (<2) dropped from 22 → 0
- Promo pages now ~150 lines each with unique content + FAQPage schema, eligible for rich results
